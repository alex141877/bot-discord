...
# Fonction pour récupérer les infos du serveur DayZ
async def get_server_info(ip, port):
    try:
        server_address = (ip, port)
        info = a2s.info(server_address)
        return info.player_count, info.max_players, info.server_name
...
async def players(ctx, *servers):
    ...
    for server in servers:
        try:
            ...
            port = int(port)
            player_count, max_players, server_name = await get_server_info(ip, port)
...
async def on_ready():
    ...
    # Affiche le contenu du module a2s pour débogage
    print("Contenu du module a2s :", dir(a2s))
